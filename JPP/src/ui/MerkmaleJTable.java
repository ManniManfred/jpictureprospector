...
public class MerkmaleJTable extends JTable implements Observer  {
  ...
  public MerkmaleJTable() {
	...
	new MerkmaleTableCellRenderer(tabellenmodell));    
    this.setzeRenderer();
  ...
  private void setzeRenderer() {
    ...
    /* DefaultRenderer ist MerkmaleTableCellRenderer, mit dem Zellen
     * gerendert werden, die Strings enthalten.
     */
    this.setDefaultRenderer(String.class,
	new MerkmaleTableCellRenderer(tabellenmodell));
    
    /* Wird eine dritte Spalte angezeigt, enthaelt sie Checkboxen und wird
     * mit dem CheckboxTableCellRenderer gerendet
     */
    if (tabellenmodell.getColumnCount() > 2) {
      this.getColumnModel().getColumn(2).setCellRenderer(new CheckboxTableCellRenderer(tabellenmodell));
    }
  }
  ...
  /**
   * Laedt die Tabelle anhand des entsprechenden <code>Observable</code>
   * neu.
   *
