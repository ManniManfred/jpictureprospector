...
public class MerkmaleTableModel extends DefaultTableModel {
  ...
  private static final String[] MERKMAL_FELDER =
  {"Schl\u00fcsselw\u00f6rter", "Beschreibung", "Thumbnail"};
  ...
  /**
   * Ein Objekt der Klasse stellt das Tabellenmodell der Merkmalstabelle
   * dar.
   *
  ...
  public MerkmaleTableModel(List<BildDokument> bilddokumente) {
    ...
    // Bilddokumente in ArrayList
    this.bilddokumente = bilddokumente;
    
    try{
      
      // Merkmalsnamen lesen, einzelne Merkmale ausschliessen.
      JPPCore core = new JPPCore();
      String[] alleMerkmalsnamen = core.getMerkmalsnamen();
  ...
  private void aktualisiereDaten() {
	...
	this.checkboxAnzeige.add(false);
      ...
      try {
	this.zeilenAnzahl =
	    JPPCore.getMerkmalsKlassen().size() - MERKMAL_FELDER.length;
	this.spaltenAnzahl =  (bilddokumente.size() == 1) ? 2 : 3;
      } catch (Exception e) {
	...
	/* Werte einfuegen:
	 * Merkmal editierbar, gleicher Wert      -> Anzeige Wert
	 *                                           Checkbox aktiviert
	 * Merkmal editierbar, ungleicher Wert    -> Vorerst nicht editierbar,
	 *                                           "verschiedene Werte",
	 *                                           Checkbox deaktiviert
	 *
	 * Merkmal nicht editierbar, gleicher Wert -> Anzeige Wert,
	 *                                            keine Checkbox
	 * Merkmal nicht editierbar, ungleicher Wert -> Anzeige "versch Werte",
	 *                                              keine Checkbox
	 */
	if (editierbar) {
  ...
  public boolean isCellEditable(int zeile, int spalte) {
    ...
    /* Entweder die Zelle liegt in der ersten Spalte
     * (Merkmalsnamen, nie editierbar),
     * in der zweiten Spalte (Editierbarkeit abhaengig von Wert in der Liste
     * editierbar)
     * oder in der dritten Spalte, die nur editierbar ist, wenn sie eine
     * Checkbox enthaelt.
     */
  ...
  public void setValueAt(Object value, int zeile, int spalte) {
     ...
     * (Merkmalswert) editierbar gemacht, wird der Wert auf false gesetzt,
     * so hatten die Merkmale unterschiedliche Werte und dies wird zurück
     * in die Zelle der Merkmalswerte geschrieben
     */
    ...
    /* Bewirkt, dass die gesamte Tabelle neu gerendert wird
     * (und damit z.B. Farbaenderungen sichtbar werden)
     */
    fireTableDataChanged();
